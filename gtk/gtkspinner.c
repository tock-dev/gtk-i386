...
ensure_paintable (GtkSpinner *spinner)
{
  ...
  spinner->paintable = gtk_path_paintable_new_from_resource ("/org/gtk/libgtk/icons/process-working.gpa");

  gtk_path_paintable_set_state (spinner->paintable,
                                spinner->spinning ? 0 : GTK_PATH_PAINTABLE_STATE_EMPTY);
                          ...
                          gboolean    spinning)
{
  ...
  if (spinner->paintable)
    gtk_path_paintable_set_state (spinner->paintable,
                                  spinning ? 0 : GTK_PATH_PAINTABLE_STATE_EMPTY);
