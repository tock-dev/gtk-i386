...
gdk_win32_app_launch_context_get_startup_notify_id (GAppLaunchContext *context,
                                                    ...
                                                    GList             *files)
{
  GdkAppLaunchContext *context_gdk = GDK_APP_LAUNCH_CONTEXT (context);
  const char *tokens[10] = {
    "win32-startup-notify",
  };
  unsigned int count = 1;

  if (context_gdk->timestamp_set)
    tokens[count++] = "foreground-window";

  g_assert (count < G_N_ELEMENTS (tokens));
  tokens[count] = NULL;

  return (count > 1) ? g_strjoinv (",", (char**)tokens) : NULL;
}
