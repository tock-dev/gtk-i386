...
gtk_gst_color_volume_from_caps (GstCaps *caps)
{
  ...
  return gdk_color_volume_new (mi.display_primaries[0].x / 50000.0, mi.display_primaries[0].y / 50000.0,
                       ...
                       GstCaps     *caps)
{
  ...
  self->color_volume = gtk_gst_color_volume_from_caps (caps);
  ...
  if (gst_is_dmabuf_memory (gst_buffer_peek_memory (buffer, 0)))
    {
      ...
      gdk_dmabuf_texture_builder_set_color_volume (builder, self->color_volume);
           ...
           gst_video_frame_map (frame, &self->v_info, buffer, GST_MAP_READ | GST_MAP_GL))
    {
      ...
      gdk_gl_texture_builder_set_color_volume (builder, self->color_volume);
  ...
  else if (gst_video_frame_map (frame, &self->v_info, buffer, GST_MAP_READ))
    {
      ...
      gdk_memory_texture_builder_set_color_volume (builder, self->color_volume);
