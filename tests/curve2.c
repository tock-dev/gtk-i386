  ...
  if (self->track)
    {
      ...
      gsk_path_get_closest_point (self->path,
                                  &GRAPHENE_POINT_INIT (x, y),
                                  INFINITY,
                                  &point,
                                  &distance);
      gsk_path_point_get_position (&point, self->path, &self->point);
      gsk_path_point_get_tangent (&point, self->path, GSK_PATH_FROM_START, &self->tangent);
      self->curvature = gsk_path_point_get_curvature (&point, self->path, GSK_PATH_FROM_START, &self->center);
  ...
  if (self->do_stroke && self->do_fill)
    {
      ...
      inside = gsk_path_in_fill (self->stroke_path, &GRAPHENE_POINT_INIT (x, y), self->fill_rule);
  ...
  if (self->start > 0 || self->end < 1)
    {
      ...
      gsk_path_measure_get_point (measure, self->start * length, &start);
      gsk_path_measure_get_point (measure, self->end * length, &end);
      builder = gsk_path_builder_new ();
      gsk_path_builder_add_segment (builder, self->orig_path, &start, &end);
...
main (int argc, char *argv[])
{
  ...
  combo = gtk_drop_down_new_from_strings ((const char *[]){"Miter", "Round", "Bevel", NULL});
