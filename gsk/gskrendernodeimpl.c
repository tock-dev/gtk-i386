                    ...
                    const graphene_point_t *offset)
{
  ...
  if (gsk_get_subpixel_resolution () < 64)
    {
      /* Hack. As long as we shift glyphs significantly when rendering, we risk
       * ink leakage and clipping, so add some slop here.
       *
       * Note that this is not technically correct, since the rendering can happen
       * in scaled context, compared to this.
       *
