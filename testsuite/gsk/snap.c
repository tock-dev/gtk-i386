...
static void
test_snap_roundtrip (void)
{
  GskSnapDirection top, right, bottom, left;
    ...
    for (right = 0; right < 4; right++)
      for (bottom = 0; bottom < 4; bottom++)
        for (left = 0; left < 4; left++)
          {
            GskRectSnap rect = gsk_rect_snap_new (top, right, bottom, left);
            GskPointSnap origin = gsk_rect_snap_get_origin_snap (rect);
            GskPointSnap opposite = gsk_rect_snap_get_opposite_snap (rect);

            g_assert_true (gsk_rect_snap_get_direction (rect, 0) == top);
            g_assert_true (gsk_rect_snap_get_direction (rect, 1) == right);
            g_assert_true (gsk_rect_snap_get_direction (rect, 2) == bottom);
            g_assert_true (gsk_rect_snap_get_direction (rect, 3) == left);

            g_assert_true (gsk_point_snap_get_direction (origin, 0) == top);
            g_assert_true (gsk_point_snap_get_direction (origin, 1) == left);
            g_assert_true (gsk_point_snap_get_direction (opposite, 1) == right);
            g_assert_true (gsk_point_snap_get_direction (opposite, 0) == bottom);

            g_assert_true (rect == gsk_rect_snap_new_from_point_snaps (origin, opposite));
         }
}
      ...
      char *argv[])
{
  ...
  g_test_add_func ("/snap/roundtrip", test_snap_roundtrip);
