                  ...
                  GError              **error)
{
  ...
  guint stroke_linejoin;
  guint stroke_linecap;
  guint fill_symbolic;
  guint fill_rule;
  GdkRGBA fill_color;
  float fill_opacity;
  ...
  guint transition_easing;
  guint animation_type;
      ...
      if (state_attr)
        {
          ...
          state = (guint) g_ascii_strtoull (state_attr, &end, 10);
  ...
  elt.attach.to = attach_to;
  elt.attach.position = attach_pos;
                    ...
                    PaintData        *data)
{
  ...
  switch (elt->animation.type)
    ...
    case GTK_PATH_ANIMATION_TYPE_AUTOMATIC:
      t = (data->time - self->animation.start_time) / (elt->animation.duration * G_TIME_SPAN_SECOND);
      break;
    default:
      g_assert_not_reached ();
    }
       ...
       PaintData        *data)
{
  ...
  out_end = self->transition.start_time + self->transition.out_duration * G_TIME_SPAN_SECOND;
