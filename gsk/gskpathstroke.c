      ...
      for (int j = 0; j < curves2->len; j++)
        {
          ...
          int count = gsk_curve_intersect (c1, c2, t1, t2, p, kind, n);
  ...
  if (segment->curve.op == GSK_PATH_CUBIC)
    {
       ...
       */
      c = segment->curve;
      if (gsk_curve_intersect (&c, &c, &t1, &t2, &p, &kind, 1) > 0)
        {
          return;
        }
    }
 ...
 */
void
gsk_contour_default_stroke (const GskContour *contour,
                            GskPathBuilder   *builder,
                            GskStroke        *stroke)
{
  StrokeData stroke_data;
  ...
  if (stroke->dash_length <= 0)
    gsk_contour_foreach (contour, stroke_op, &stroke_data);
  else
    gsk_contour_dash (contour, stroke, stroke_op, &stroke_data);
                            ...
                            float             miter_limit)
{
  ...
  gsk_contour_foreach (contour, offset_op, &offset_data);
